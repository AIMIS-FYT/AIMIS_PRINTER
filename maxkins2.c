		      ...
		      KINEMATICS_INVERSE_FLAGS * iflags)
{
    ...
    // U correction
    double zv = joints[6] * sin(d2r(joints[3]));
    double xv = joints[6] * cos(d2r(joints[3]));
...
int rtapi_app_main(void) {
    int result;
    comp_id = hal_init("maxkins2");
    if(comp_id < 0) return comp_id;

    haldata = hal_malloc(sizeof(struct haldata));

    result = hal_pin_float_new("maxkins2.pivot-length", HAL_IO, &(haldata->pivot_length), comp_id);
